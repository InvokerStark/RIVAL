...
class RewardModelWorker(Worker):
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            ...
            scores = torch.cat(output, dim=0)  # raw: [bs] ====> [bs, 3]
            if len(scores.shape) == 2:
                print(f"compute_rm_score before scale----> scores: {scores.shape} // {scores[0]} // {scores[1]}")
                # normalize rank reward
                rank_score = scores[:, 0]
                bleu_rouge_score = scores[:, 1:]
                min_val, _ = torch.min(rank_score, dim=0)
                max_val, _ = torch.max(rank_score, dim=0)
                normalized_rank_score = ((rank_score - min_val) / (max_val - min_val + 1e-8)).unsqueeze(1)
                scores_multi = torch.cat((normalized_rank_score, bleu_rouge_score), dim=1)  # [bs, 3]
                scores = scores_multi.sum(dim=1, keepdim=True).squeeze(1)  # [bs]
                scores = scores / 2
                print(f"compute_rm_score after scale ----> scores: {scores.shape} // {scores[0]} // {scores[1]}")
            else:
                print(f"compute_rm_score before scale----> scores: {scores.shape}")
                # scores = scores.sum(dim=0, keepdim=True) # [bs]
                scores = scores
                print(f"compute_rm_score after scale ----> scores: {scores.shape}")
            if use_dynamic_bsz:
